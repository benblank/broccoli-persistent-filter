...
describe('Filter', function() {
  ...
  it('processes files with extensions included in `extensions` list by default', function() {
   let filter = MyFilter('.', { extensions: ['c', 'cc', 'js']});
    ...
    expect(filter.canProcessFile('foo.c', Entry.fromPath('foo.c'))).to.equal(true);
    expect(filter.canProcessFile('test.js', Entry.fromPath('test.js'))).to.equal(true);
    expect(filter.canProcessFile('blob.cc', Entry.fromPath('blob.cc'))).to.equal(true);
    expect(filter.canProcessFile('twerp.rs', Entry.fromPath('twerp.rs'))).to.equal(false);
  });
  ...
  it('getDestFilePath returns null for directories when extensions is null', function() {
    ...
    expect(filter.getDestFilePath('a/bar/', Entry.fromPath('a/bar/'))).to.equal(null);
    expect(filter.getDestFilePath('a/bar/bar.js', Entry.fromPath('a/bar/bar.js'))).to.equal('a/bar/bar.js');
  });
  ...
  it('getDestFilePath returns null for directories with matching extensions', function() {
    ...
    expect(filter.getDestFilePath('a/loader.js/', Entry.fromPath('a/loader.js/'))).to.equal(null);
    expect(filter.getDestFilePath('a/loader.js/loader.js', Entry.fromPath('a/loader.js/loader.js'))).to.equal('a/loader.js/loader.js');
  });
  ...
  it('replaces matched extension with targetExtension by default', function() {
    ...
    expect(filter.getDestFilePath('foo.c', Entry.fromPath('foo.c'))).to.equal('foo.zebra');
    expect(filter.getDestFilePath('test.js', Entry.fromPath('test.js'))).to.equal('test.zebra');
    expect(filter.getDestFilePath('blob.cc', Entry.fromPath('blob.cc'))).to.equal('blob.zebra');
    expect(filter.getDestFilePath('twerp.rs', Entry.fromPath('twerp.rs'))).to.equal(null);
  ...
  describe('on rebuild', function() {
    ...
    it.only('handles renames', co.wrap(function* () {
      debugger;
       let builder = makeBuilder(Rot13Filter, fixturePath('a'), awk => {
        sinon.spy(awk, 'processString');
        return awk;
